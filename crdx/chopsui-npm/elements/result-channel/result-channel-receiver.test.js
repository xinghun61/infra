...
import {TYPES, ResultChannelSender} from './result-channel-sender.js';
...
test('send after close', async () => {
  ...
  const receiver = new ResultChannelReceiver(url);
  const channel = new BroadcastChannel(url);
  channel.postMessage({type: TYPES.DONE});
  channel.postMessage({type: TYPES.RESULT, payload: 'post facto'});
  const results = [];
  for await (const result of receiver) {
    results.push(result);
  }
