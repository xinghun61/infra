...
export class TSMonClient {
  ...
  _measure(metric, value, fields, cumulative) {
    const expectedFieldLen = metric.info.Fields ? metric.info.Fields.size : 0;
    const gotFieldLen = fields ? fields.size : 0;
    if (expectedFieldLen != gotFieldLen) {
      throw `field count ${metric.name}: ${expectedFieldLen} != ${gotFieldLen}`;
    }
...
export class TSAdder extends TSMetric {
  /**
   * @param {*} v value
...
export class TSSetter extends TSMetric {
  /**
   * @param {*} v value
   * @param {Map<String, *>} fields map of field name to field value
   */
...
export class TSCumulativeDistribution extends TSAdder {
...
export class TSNonCumulativeDistribution extends TSSetter {
  constructor(tsMon, vt, name, desc, metadata, fields, bucketer) {
    super(tsMon, vt, name, desc, metadata, fields);
    this.bucketer = bucketer || TSMonClient.geometricBucketer();
  }
...
export class Bucketer {
  constructor(width, growthFactor, numFiniteBuckets, opt_scale) {
    ...
    if (width != 0 && growthFactor != 0) {
      throw new Error(
        `Bucketer must be created with either a width or a growth factor, not both.`
      );
    }
    if (width < 0) {
    ...
    this._lowerBounds.forEach((curr, i) => {
      ...
      if (next <= curr) {
        throw new Error(
          `bucket boundaries must be monotonically increasing: ${curr}, ${next}`
        );
      }
    });
  }
