...
def GenTests(api):
  swig = api.third_party_packages.swig
  test_refs = api.gitiles.make_refs_test_data(*REFS)

  def GenTest(platform_name, bits, platform_suffix, exists=False):
    package_name = swig.PACKAGE_PREFIX + platform_suffix
    package_tag = 'version:%s%s' % ('3.0.16', swig.PACKAGE_VERSION_SUFFIX)
    test_data = (
  ...
  yield (
      ...
      api.platform('mac', 64) +
      api.properties(dry_run=False) +
      api.properties(git_release_tag='rel-3.0.17')
  )
