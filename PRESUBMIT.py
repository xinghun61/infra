...
DISABLED_PROJECTS = [
    ...
    'appengine/chromium_git_access/tools',
...
def PylintChecks(input_api, output_api):  # pragma: no cover
  ...
  appengine_env_path = input_api.os_path.join(
      input_api.os_path.dirname(infra_root), 'google_appengine')
  ...
  extra_syspaths = [appengine_env_path, venv_path]
  ...
  for root_path in sorted(dirty_roots):
    ...
    # Hack for appengine. :(
    if 'appengine' in root_path:
      syspaths += appengine_lib_paths
