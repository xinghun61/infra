...
HEADERS_WITH_CONTENT_TYPE['Content-Length'] = str(0)
...
class GerritAgentTestCase(unittest.TestCase):
  ...
  def test_request_throttled(self, mock_method, time_mock_method, sleep_mock):
    ...
    gerrit_throttled._request(method='GET', request_path='/accounts/self')
    # We expect to sleep for 0.1 seconds (see throttle parameter above)
  ...
  def test_query(self, mock_method):
    ...
    result = self.gerrit._query(project='test', with_labels=False,
    ...
    mock_method.assert_called_once_with(
        ...
        params=None,
        url=('https://chromium-review.googlesource.com/a/changes/'
             '?q=project:test+owner:test@chromium.org&o=MESSAGES'),
        headers=HEADERS)
  ...
  def test_query_unexpected_response(self, mock_method):
    mock_method.return_value = _create_mock_return(None, 400)
    self.assertRaises(gerrit_api.UnexpectedResponseException,
                      self.gerrit._query, 'a', with_messages=False,
                      with_labels=False, with_revisions=False)

  @mock.patch.object(requests.Session, 'request')
  def test_get_pending_issues(self, mock_method):
    mock_method.return_value = _create_mock_return(
        '%s%s' % (GERRIT_JSON_HEADER, json.dumps([TEST_CHANGE_INFO])), 200)
    result = self.gerrit.get_pending_issues('test')
    mock_method.assert_called_once_with(
        data=None,
        method='GET',
        params=None,
        url=('https://chromium-review.googlesource.com/a/changes/?q=project:'
             'test+query:pending_cls&o=MESSAGES&o=LABELS&o=ALL_REVISIONS'),
        headers=HEADERS)
    self.assertEquals(result, [TEST_CHANGE_INFO])

  @mock.patch.object(requests.Session, 'request')
  def test_get_commit_ready_issues(self, mock_method):
    mock_method.return_value = _create_mock_return(
        '%s%s' % (GERRIT_JSON_HEADER, json.dumps([TEST_CHANGE_INFO])), 200)
    result = self.gerrit.get_commit_ready_issues('test')
    mock_method.assert_called_once_with(
        data=None,
        method='GET',
        params=None,
        url=('https://chromium-review.googlesource.com/a/changes/?q=project:'
             'test+query:commit_ready_cls&o=LABELS&o=ALL_REVISIONS'),
        headers=HEADERS)
    self.assertEquals(result, [TEST_CHANGE_INFO])
