...
def MakeIssueService(project_service, config_service, cache_manager, my_mox):
  ...
  for table_var in [
      ...
      'issueformerlocations_tbl', 'comment_tbl',
      # SOON(jrobbins): 'commentcontent_tbl',
      'issueupdate_tbl', 'attachment_tbl',
      'reindexqueue_tbl', 'localidcounter_tbl']:
...
class IssueServiceTest(unittest.TestCase):
  ...
  def testDeserializeComments_StringsInCommentContent(self):
    comment_rows, amendment_rows, attachment_rows = self.SetUpCommentRows()
    # SOON(jrobbins): commentcontent_rows = [
    # (7890101, 'replacement content', 'replacement msg')]
    comments = self.services.issue._DeserializeComments(
        comment_rows, amendment_rows, attachment_rows)
    self.assertEqual(1, len(comments))
  ...
  def SetUpGetComments(self, issue_ids):
    ...
    # SOON(jrobbins): self.services.issue.commentcontent_tbl.Select(
  ...
  def SetUpInsertComment(self, comment_id, is_spam=False,
    ...
    # SOON(jrobbins): self.services.issue.commentcontent_tbl.InsertRow(
