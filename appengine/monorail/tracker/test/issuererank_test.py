...
class IssueRerankFunctionsTest(unittest.TestCase):

  def testGetInsertRankings(self):
    lower = [(1, 0)]
    higher = [(2, 10)]
    moved_ids = [3]
    ret = issuererank._GetInsertRankings(lower, higher, moved_ids)
  ...
  def testGetInsertRankings_NoRoom(self):
    max_ranking, issuererank.MAX_RANKING = issuererank.MAX_RANKING, 1
    lower = [(1, 0)]
    higher = [(2, 1)]
    moved_ids = [3]
    ret = issuererank._GetInsertRankings(lower, higher, moved_ids)
    self.assertIsNone(ret)
    issuererank.MAX_RANKING = max_ranking
