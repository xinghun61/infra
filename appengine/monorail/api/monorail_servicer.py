...
class MonorailServicer(object):
  ...
  def AssertBaseChecks(self, mc, request, metadata):
    ...
    self.AssertWhitelistedOrXSRF(mc, request, metadata)

  def AssertWhitelistedOrXSRF(self, mc, request, metadata):
    ...
    token = request.trace.token  # TODO(jrobbins): Phase out after JS update.
    if XSRF_TOKEN_HEADER in metadata:
      token = metadata[XSRF_TOKEN_HEADER]
    xsrf.ValidateToken(
