...
class MonitoringUploadV2Test(testing.AppengineTestCase):
  app_module = main.app  # for testing.AppengineTestCase

  def setUp(self):
    super(MonitoringUploadV2Test, self).setUp()
  ...
  def tearDown(self):
    ...
    super(MonitoringUploadV2Test, self).tearDown()
  ...
  def test_creates_event_mon_event_correctly(self):
    response = self.test_app.post('/internal/v2/monitoring/upload',
            json.dumps(DATA))
  ...
  def test_returns_400_on_missing_request_params(self):
    self.test_app.post('/internal/v2/monitoring/upload',
  ...
  def test_does_not_crash_on_missing_required_fields_in_json(self):
    ...
    response = self.test_app.post('/internal/v2/monitoring/upload',
            json.dumps(MISSING_FIELDS_DATA))
    self.assertEqual(200, response.status_int)
    events = self.read_event_mon_file()
    self.assertEqual(1, len(events))
