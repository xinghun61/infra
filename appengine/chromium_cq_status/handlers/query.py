...
def execute_query(
    project, key, begin, end, tags, fields, count, cursor): # pragma: no cover
  ...
  while more and len(results) < count:
    if key and not filters and count > 0:
      record = Record.get_by_id(key, namespace=project)
      records = [record] if record else []
      more = False
    else:
      query = Record.query(namespace=project)
      query.filter(*filters).order(-Record.timestamp)
