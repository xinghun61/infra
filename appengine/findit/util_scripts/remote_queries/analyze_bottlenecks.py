  ...
  for i in range(len(record) - 1):
    ...
    if record[i][1].endswith('.start'):
      ...
      if label_pair[1] in loaded_records[k].keys():
        ...
        end = loaded_records[k][label_pair[1]] or datetime.datetime(2010, 1, 1)
...
def _SummarizeRecordCustom(from_regex, to_regex):
  ...
  def inner(loaded_records, coalesce, record, k, transitions, adjacent,
            examples, totals):
    start_index = 0
...
"""
  ...
  pre_aggregation.add_option(
      '--exclude_android',
      action='store_true',
      help='exclude records for android platforms')
  pre_aggregation.add_option(
      '--allowed_masters',
      default='',
      help='Comma'
  ...
  coalesce_opts.add_option(
      ...
      action='store_true',
      default=False,
      help='Take into account only the '
  ...
  post_aggregation.add_option(
      ...
      'adjacent_count.')
  post_aggregation.add_option(
      '-t', '--top', type='int', default=100, help='How many rows to display.')
  post_aggregation.add_option(
      '-l',
      '--label_filter',
      help='Only display '
  ...
  post_aggregation.add_option(
      ...
      type='int',
      default=10,
      help='Exclude transitions that occur '
  ...
  display_opts.add_option(
      ...
      action='store_const',
      const='json',
      help='Dump data as JSON to STDOUT.')
  display_opts.add_option(
      '--csv',
      dest='output_format',
      action='store_const',
      const='csv',
      help='Produce a comma-separated values '
  ...
  display_opts.add_option(
      ...
      action='store_const',
      const='none',
      help='Do not display results (e.g. if '
  ...
  global_stats.add_option(
      '--all_globals', action='store_true', help='Implies '
      'the following:')
...
def main():
  ...
  # Results
  aggregated_transitions = _AggregateTransitions(
      options, *_LoadFilterAndSortRecords(options))

  if options.raw_values_only:
    aggregated_transitions = [
        x for x in aggregated_transitions if any(x['values'])
    ]
  else:
