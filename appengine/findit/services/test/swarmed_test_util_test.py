...
class SwarmedTestUtilTest(wf_testcase.WaterfallTestCase):
  ...
  @mock.patch.object(
      swarmed_test_util, 'GetIsolatedOutputForTask', return_value={})
  ...
  @mock.patch.object(
      isolate, 'DownloadFileFromIsolatedServer', return_value=('content', None))
  def testGetSwarmingTaskFailureLog(self, _):
    outputs_ref = {
    ...
    result, error = swarmed_test_util.GetSwarmingTaskFailureLog(
        outputs_ref, None)
  ...
  def testGetIsolatedOutputForTaskIsolatedError(self, *_):
    self.assertIsNone(swarmed_test_util.GetIsolatedOutputForTask(None, None))
  ...
  def testGetIsolatedOutputForTask(self, mock_fn, _):
    task_id = '2944afa502297110'
    result = swarmed_test_util.GetIsolatedOutputForTask(task_id, None)
