...
class CheckTrybotMappingTest(wf_testcase.WaterfallTestCase):
  ...
  def testGetsupportedMasters(self):
    self.UpdateUnitTestConfigSettings(
        ...
        override_data={'supported_masters': {'chromium.linux': {}}})
    self.assertEqual(
        ['chromium.linux'],
        check_trybot_mapping._GetSupportedMasters())

  @mock.patch.object(buildbot, 'ListBuildersOnMaster',
                     return_value=['builder1', 'builder2'])
  ...
  def testGetCoveredBuilders(self):
    ...
    self.assertEqual(
        expected,
        check_trybot_mapping._GetCoveredBuilders(config))
  ...
  @mock.patch.object(check_trybot_mapping, '_GetAllFinditBuildersOnTryservers',
                     return_value={'tryserver1': ['trybot3', 'trybot4'],
                                   'tryserver2': ['trybot3']})
  def testGetUnusedVariableBuilders(self, _):
    ...
    http_client = mock.create_autospec(RetryHttpClient)
    self.assertEqual({'tryserver1': ['trybot4']},
                     check_trybot_mapping._GetUnusedVariableBuilders(
                         config, http_client))

  @mock.patch.object(check_trybot_mapping, '_GetSupportedMasters',
                     return_value=[])
  @mock.patch.object(check_trybot_mapping, '_GetCoveredBuilders',
                     return_value={})
  @mock.patch.object(check_trybot_mapping, '_GetDiffBetweenDicts',
                     return_value={})
  @mock.patch.object(check_trybot_mapping, '_GetUnusedVariableBuilders',
                     return_value={})
  def testGet(self, *_):
    self.mock_current_user(user_email='test@chromium.org', is_admin=True)
    response = self.test_app.get('/check-trybot-mapping',
                                 params={'format': 'json'})
