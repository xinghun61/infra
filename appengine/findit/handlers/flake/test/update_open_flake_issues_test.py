...
class UpdateOpenFlakeIssuesTest(WaterfallTestCase):
  app_module = webapp2.WSGIApplication([
      ('/flake/cron/update-open-flake-issues',
  ...
  def setUp(self):
    ...
    self.url = '/flake/cron/update-open-flake-issues'
...
class UpdateOpenFlakeIssuesTaskTest(WaterfallTestCase):
  app_module = webapp2.WSGIApplication([
      ('/flake/task/update-open-flake-issues',
  ...
  def testUpdateOpenFlakeIssuesTask(self, mocked_service,
    ...
    response = self.test_app.get('/flake/task/update-open-flake-issues')
    self.assertEqual(200, response.status_int)
