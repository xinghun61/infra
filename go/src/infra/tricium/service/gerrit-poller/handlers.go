...
func convertToAnalysisTasks(changes []gerrit.ChangeInfo) []*common.AnalysisTask {
	...
	for _, c := range changes {
		...
		for k, v := range files {
			fc = append(fc, common.FileChangeDetails{
				Path:   k,
				Status: v.Status,
			})
		}
...
// enqueueChangesAsServiceTasks enqueues the given analysis tasks.
func enqueueChangesAsServiceTasks(ctx context.Context, tasks []*common.AnalysisTask) error {
