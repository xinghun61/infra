...
func Example() {
	...
	accountID := "Account1"
	s.AddAccount(accountID, accountConfig, nil)
	...
	s.AddRequest(requestID, request, time.Now())

	// Inform the scheduler of the existence of an idle worker.
	workerID := "Worker1"
	s.MarkIdle(workerID, []string{"Label2"}, time.Now())
	...
	s.NotifyRequest("Request1", "Worker1", time.Now())
	...
	s.NotifyRequest("Request2", "Worker1", time.Now())
