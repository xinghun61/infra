...
const (
	...
	nobodyOncallSubject = `RotaNG - Nobody on-call for {{.RotaName}} at {{.ShiftEntry.StartTime}}`
	nobodyOncallBody    = `Nobody is on-call for {{.RotaName}} shift {{.ShiftEntry.StartTime}} - {{.ShiftEntry.EndTime}}.
...
func init() {
	...
	opts := handlers.Options{
		...
		NobodyEmail: handlers.NobodyEmail{
			Subject: nobodyOncallSubject,
			Body:    nobodyOncallBody,
		},
		ProdENV:   prodENV,
		AuthGroup: authGroup,
	}
