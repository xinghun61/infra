...
func Export(ctx context.Context, td *tabledef.TableDef, packageName, structName, out string) error {
	...
	a.addStruct("", fmt.Sprintf("the schema for %q.", tableDefName), td.Fields)
...
func (a *analyzer) addPackage(name, path string) {
...
func (a *analyzer) addStruct(fieldName, comment string, schema []*tabledef.FieldSchema) string {
...
func (a *analyzer) makeFieldEntry(fs *tabledef.FieldSchema) *fieldEntry {
	...
	case tabledef.Type_TIMESTAMP:
		a.addPackage("time", "time")
		typ = "time.Time"
	case tabledef.Type_RECORD:
		typ = "*" + a.addStruct(fs.Name, "", fs.Schema)
	case tabledef.Type_DATE:
		a.addPackage("civil", "cloud.google.com/go/civil")
		typ = "civil.Date"
