...
class _Bucketer(object):
  ...
  def __init__(self, width, growth_factor, num_finite_buckets):
    ...
    For geometric buckets::
      ...
      The i'th bucket covers the interval [factor^(i-1), factor^i) where i
      ranges from 1 to num_finite_buckets inclusive.

      bucket number                   lower bound    upper bound
      i == 0 (underflow)              -inf           1
      1 <= i <= num_buckets           factor^(i-1)   factor^i
      i == num_buckets+1 (overflow)   factor^(i-1)   +inf
    """
  ...
  def _exponential_bounds(self):
    return [
        self.growth_factor ** i for i in xrange(self.num_finite_buckets + 1)]
